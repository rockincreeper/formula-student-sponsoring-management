...
public class SponsoringFactory {
    public static SponsoringAggregate createSponsoringAggregate(UUID teamId, UUID teamMemberId, UUID userId, double budgetForSponsoring) {
        if(budgetForSponsoring <= 0){
            throw new IllegalArgumentException("The budget for sponsoring is invalid");
        }
        return new SponsoringAggregate(teamId, teamMemberId, userId, budgetForSponsoring);
    }
}
